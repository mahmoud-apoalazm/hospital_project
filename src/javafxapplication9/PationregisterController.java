 ...
 * @author aboal
 */
public class PationregisterController implements Initializable {
private Connection con = null;
    ...
    public PationregisterController() throws ClassNotFoundException {
        try {
    ...
    public void handleButtonClicks(javafx.event.ActionEvent ae) throws SQLException {
        if (ae.getSource() == btnMain) {    ///button to go back to main page
            ...
            FxmlLoader.loadStage("pationtlogin.fxml");
            ((Node) (ae.getSource())).getScene().getWindow().hide();
    ...
    private String saveData() throws SQLException {
        try {
            String st;
            st = "INSERT INTO patients ( complain, department, first_name, last_name, dob, phone_number, username, password, report, disease, medicine, cured) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
